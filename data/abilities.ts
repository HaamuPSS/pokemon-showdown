...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	petrificate: {
		onStart(pokemon) {
			...
			for (const target of pokemon.adjacentFoes()) {
				if (!activated) {
					this.add('-ability', pokemon, 'Petrificate', 'boost');
					activated = true;
				}
				if (target.volatiles['substitute']) {
					this.add('-immune', target);
	...
	fullmetalclaws: {
		...
		onModifyType(move, pokemon) {
			...
			if (move.type === 'Normal' && !noModifyType.includes(move.id) && !(move.isZ && move.category !== 'Status')) {
				move.type = 'Steel';
				move.fullmetalBoosted = true;
			}
		},
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) {
			if (move.fullmetalBoosted) return this.chainModify([4915, 4096]);
		},
	...
	shroomtrip: {
		onStart(source) {
			this.field.addPseudoWeather('shroomtrip');
		},
		onEnd(pokemon) {
			for (const target of this.getAllActive()) {
				if (target === pokemon) continue;
			...
			this.field.removePseudoWeather('shroomtrip');
		},
		name: "Shroom Trip",
		gen: 8,
		num: -12,
